...
+ (void)mungeThis:(RawMessage*)message {
    ...
    static const NSString *prefix, *suffix;
    ...
    NSData *data = [[NSString stringWithFormat:@"%@%@%@", prefix, message.content, suffix] dataUsingEncoding:NSUTF8StringEncoding];
    
    NSDictionary * attributes = @{NSFontAttributeName: [UIFont fontWithName:@"Source Sans Pro" size:12.0]};
    
    NSAttributedString *tmp = [[NSAttributedString alloc] initWithData:data options:@{ NSDocumentTypeDocumentAttribute: NSHTMLTextDocumentType } documentAttributes: &attributes error:nil];
    
    NSCharacterSet *charSet = [NSCharacterSet whitespaceCharacterSet].invertedSet;

    // find start of message, skipping all whitespace
    NSRange range = [tmp.string rangeOfCharacterFromSet:charSet];
    unsigned int start = (range.length > 0) ? (int)range.location : 0;
    // find end of message, omitting all trailing whitespace
    range = [tmp.string rangeOfCharacterFromSet:charSet options:NSBackwardsSearch];
    unsigned int len = (range.length > 0) ? (int)range.location - start : (int)tmp.string.length - start;

    message.attributedString = [tmp attributedSubstringFromRange:NSMakeRange(start, len)];
